...
int main() {
    ...
    fout << std::right << std::setw(40) << "CPA Resort Hotel Room Charge\n"
    ...
    do {        
        ...
        fout << std::left << std::setw(20) << guestName
            << std::left << std::setw(20) << roomTypeOutput
            << std::right << std::setw(18) << roomCharges << std::endl;
        // Ask for next entry
        enterGuest = validateReply("Do you want to enter another guest? Answer yes/no: ");
        std::cin.ignore(80, '\n');
    } while (enterGuest);
    std::cout << "\n";
    fout << std::left << std::setw(40) << "Total Charges: "
        << std::right << std::setw(18) << totalCharges << std::endl;
...
int getValidDays() {
    ...
    std::cin >> daysAmount;
    while (daysAmount < 1 || daysAmount > 31 || std::cin.fail()) {
        std::cin.clear();
        std::cin.ignore(80, '\n');
        std::cout << "Wrong input, enter a number between 1 and 31: ";
        std::cin >> daysAmount;
    }
    return daysAmount;
}
